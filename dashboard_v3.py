...
def main():
    # Header
    st.markdown('<p class="main-header">🎬 Streaming Tracker V3.0</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Watch Now • Discover</p>', unsafe_allow_html=True)
    
    # Create tabs
    tab1, tab2 = st.tabs(["📺 Watch Now", "🔍 Discover"])
...
def render_watch_now_tab():
    st.header("📺 Watch Now - Trending with Reviews")
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔄 Refresh Data", key="refresh_watch"):
            st.cache_data.clear()
            st.rerun()
    with col2:
...
def render_discover_tab():
    st.header("🔍 Discover - Browse by Category")
    ...
    with col2:
        ...
        st.metric("⭐ Classics", classics_count)
    ...
    # Sidebar filters
    st.sidebar.header("🔍 Discover Filters")
    ...
    display_df['Rating'] = display_df['imdb_rating'].apply(lambda x: f"⭐ {x:.1f}" if pd.notna(x) else "N/A")
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔄 Refresh Data", key="refresh_discover"):
            st.cache_data.clear()
            st.rerun()
    with col2:
        st.caption(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
