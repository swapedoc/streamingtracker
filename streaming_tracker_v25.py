...
class Config:
    ...
    # Quota management - INCREASED LIMITS
    MAX_VIDEOS_PER_PLATFORM = 10  # Increased from 3
    TRENDING_LIMIT = 40           # Increased from 20
    UNDERDOG_LIMIT = 20           # Increased from 10
    GENRE_LIMIT = 15              # New: items per genre
    CLASSICS_LIMIT = 20           # New: Top rated classics
    ENABLED_GENRES = ['Action', 'Horror', 'Thriller']  # Which genres to track
            ...
            try:
                response = requests.get(
                    ...
                    params={
                        ...
                        'with_watch_providers': '8|119|350|337|220',  # Our platforms
                        'watch_region': 'IN',
                        'page': 1
                    },
                    timeout=30,
...
class ScoreComputer:
    ...
    def compute_all(self):
        ...
        for content in content_result.data:
            ...
            if not providers or platform_id not in providers:
                print(f"\n🚫 {title} - No longer on {content['platform']}, removing...")
